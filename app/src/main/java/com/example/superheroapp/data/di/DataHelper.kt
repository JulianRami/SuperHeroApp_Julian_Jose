    ...
    fun generateSuperheroes(): List<Superhero> {
        ...
        for (i in 0 until 20) {
            val superhero = Superhero(
                ...
                friends = (0..Random.nextInt(0, 5)).map { Random.nextInt(0, 20) }.distinct().take(
                    Random.nextInt(1, 6)),
                mainEnemy = enemies.random(),
                enemies = (0..Random.nextInt(0, 5)).map { Random.nextInt(0, enemies.size) }.distinct().take(
                    Random.nextInt(1, 6)),
                locations = (0..Random.nextInt(0, 3)).map { Random.nextInt(0, locations.size) }.distinct().take(
                    Random.nextInt(1, 4))
            )
